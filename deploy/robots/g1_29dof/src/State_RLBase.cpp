...
REGISTER_OBSERVATION(keyboard_velocity_commands)
{
    ...
    static std::unordered_map<std::string, std::vector<float>> key_commands = {
        ...
        {"q", {0.0f, 0.0f, 0.02f}},   // Turn left (50% max - CRITICAL: no ang curriculum)
        {"e", {0.0f, 0.0f, -0.02f}}   // Turn right (50% max - CRITICAL: no ang curriculum)
    };
    ...
    if (key_pressed)
    {
        ...
        std::vector<float> target_cmd = key_commands[key];
    ...
    {
        // When no key is pressed, immediately stop all motion - zero out everything
